...
void ledTask (void const *arg) {
  while (1) {
    ...
    //osEvent evt = osSignalWait(0x0001, 3000);
		while(osSignalWait(0x0001, 3000).status == osEventSignal){
				redLEDOnOff(LED_OFF);
				osSignalWait(0x0001, osWaitForever);
				osDelay(200);               // delay 200ms
				osSignalClear (t_led, 0x0001); // discard pending notifications
				redLEDOnOff(LED_ON);
		}
